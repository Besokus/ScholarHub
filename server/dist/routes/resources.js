...
router.post('/', (req, res) => {
    ...
    if (!title || !summary || !courseId)
        return res.status(400).json({ message: 'Invalid' });
    const id = `r${Date.now()}`;
    const uploaderId = (req.header('X-User-Id') || '').trim() || undefined;
    const item = { id, title, summary, courseId, type: type || 'PDF', size: size || '0MB', downloadCount: 0, uploaderId, uploaderName: uploaderId ? `用户${uploaderId}` : '系统', fileUrl };
