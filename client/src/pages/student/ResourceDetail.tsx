...
export default function ResourceDetail() {
  ...
  const [meta, setMeta] = useState<any>({ title: '', uploader: { name: '' }, size: '', type: '', summary: '' })
  useEffect(() => {
    const load = async () => {
      if (!id) return
      try {
        const r = await ResourcesApi.detail(id)
        setMeta({ title: r.title, uploader: { name: '系统' }, size: r.size, type: r.type, summary: r.summary })
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="p-6">
          ...
          <button onClick={download} className="mt-3 px-4 py-2 bg-indigo-600 text-white rounded-lg">下载</button>
        </Card>
      </div>
    </div>
  )
