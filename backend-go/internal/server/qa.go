...
func (q *QAController) List(c *gin.Context) {
    ...
    pageSize, _ := strconv.Atoi(c.Query("pageSize")); if pageSize < 1 { pageSize = 15 }
    uid := c.GetInt("user_id")
    var list []models.Question
    tx := q.db
    if status == "unanswered" { tx = tx.Where("status = ?", "UNANSWERED") }
    if my == "1" && uid != 0 { tx = tx.Where("\"studentId\" = ?", uid) }
    if sort == "hot" { tx = tx.Order("hot desc") } else { tx = tx.Order("\"createTime\" desc") }
    var total int64
...
func (q *QAController) Detail(c *gin.Context) {
    ...
    if err := q.db.First(&item, id).Error; err != nil { c.JSON(http.StatusNotFound, respErr(1006, "not_found")); return }
    c.JSON(http.StatusOK, respOk(item))
}

func (q *QAController) Create(c *gin.Context) {
    uid := c.GetInt("user_id")
    var req struct{ CourseID int; Title string; ContentHTML string; Images []string }
    ...
    item.Images = images
    if err := q.db.Create(&item).Error; err != nil { c.JSON(http.StatusInternalServerError, respErr(1004, "db_error")); return }
    c.JSON(http.StatusOK, respOk(item))
}

func toJSONB(arr []string) []byte {
    if len(arr) == 0 { return []byte("[]") }
    b := "["
    for i, s := range arr { if i > 0 { b += "," }; b += "\"" + s + "\"" }
    b += "]"
    return []byte(b)
}
